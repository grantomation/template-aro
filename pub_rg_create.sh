...
export SCOPE_PUB=$(az group create -n $PUB_RG -l $LOCATION --query id -o tsv)

az role assignment create --assignee $AAD_APP_CLIENT_ID --role contributor --scope $SCOPE_PUB
az role assignment create --assignee $AAD_APP_CLIENT_ID --role "User Access Administrator" --scope $SCOPE_PUB
